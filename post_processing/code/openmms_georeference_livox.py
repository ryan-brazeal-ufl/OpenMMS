...
def geoRefCalcSocsX(azi, vert, dist):
    return math.sin(math.radians(vert)) * math.cos(math.radians(azi)) * dist


@vectorize(['float32(float32, float32, float32)'], target='cpu', nopython=True)
def geoRefCalcSocsY(azi, vert, dist):
    return math.sin(math.radians(vert)) * math.sin(math.radians(azi)) * dist


@vectorize(['float32(float32, float32)'], target='cpu', nopython=True)
def geoRefCalcSocsZ(vert, dist):
    return math.cos(math.radians(vert)) * dist
...
def geoReferenceCalcs(points_timing, points_azi, points_vert, points_dist, points_intens, points_return, navinterpSinLat, navinterpCosLat, navinterpSinLon, navinterpCosLon, navinterpEllH, navinterpGeoidSep, navinterpSinR, navinterpCosR, navinterpSinP, navinterpCosP, navinterpSinH, navinterpCosH, navinterpXg, navinterpYg, navinterpZg, navinterpTa, navinterpZar, navinterpDr, navinterpVh, navinterpVu, params):
    ...
    x0 = geoRefCalcSocsX(azi0, vert0, dist0)
    y0 = geoRefCalcSocsY(azi0, vert0, dist0)
    z0 = geoRefCalcSocsZ(vert0, dist0)

    # i = 250000
    # print(times[i],azi0[i],vert0[i],dist0[i],x0[i],y0[i],z0[i],Lat[i],Lon[i],R[i],P[i],H[i])
    # sys.exit(0)
...
def outputPrcsPoints_a(lock,filePath,csv_filename, x, y, z, times, intens, retur, dist, heading, vert, totalAccel, ZangRate, deltaRoll):

    csv = open(filePath / (csv_filename + "_temp.csv"),"w")
    csv.write("//X,Y,Z,Time,Intensity,Return Number,range,heading,total_accel,Z_ang_rate,delta_roll\n")
    ...
    for i in range(0,len(x)):
        csv.write("{:0.3f}".format(x[i]) + "," + "{:0.3f}".format(y[i]) + "," + "{:0.3f}".format(z[i]) + "," + "{:0.2f}".format(times[i]) + "," + str(intens[i]) + "," + str(retur[i]) + "," + "{:0.3f}".format(dist[i]) + "," + "{:0.1f}".format(heading[i]) + "," + "{:0.3f}".format(totalAccel[i]) + "," + "{:0.3f}".format(ZangRate[i]) + "," + "{:0.3f}".format(deltaRoll[i]) + "\n")
...
def outputPrcsPoints_a_verbose(lock,filePath,csv_filename, X, Y, Z, times, intens, retur, dist, vert, azi, x0, y0, z0, R, P, H, Es, Ns, hs):

    csv = open(filePath / (csv_filename + "_temp.csv"),"w")
    csv.write("//X,Y,Z,Time,Intensity,range,laser_num,azimuth,x_socs,y_socs,z_socs,roll,pitch,heading,scanner_x,scanner_y,scanner_z\n")
    ...
    for i in range(0,len(X)):
        if retur[i] == 1:
            csv.write("{:0.3f}".format(X[i]) + "," + "{:0.3f}".format(Y[i]) + "," + "{:0.3f}".format(Z[i]) + "," + "{:0.7f}".format(times[i]) + "," + str(intens[i]) + "," + "{:0.3f}".format(dist[i]) + "," + str(vert[i]) + "," + str(azi[i]) + "," + "{:0.4f}".format(x0[i]) + "," + "{:0.4f}".format(y0[i]) + "," + "{:0.4f}".format(z0[i]) + "," + "{:0.4f}".format(R[i]) + "," + "{:0.4f}".format(P[i]) + "," + "{:0.4f}".format(H[i]) + "," + "{:0.3f}".format(Es[i]) + "," + "{:0.3f}".format(Ns[i]) + "," + "{:0.3f}".format(hs[i]) + "\n")
...
def outputPrcsPoints_a_verbose2(lock,filePath,csv_filename, X, Y, Z, time, retur, x0, y0, z0, R, P, H, Xg, Yg, Zg, Lat, Lon, Xg_offset, Yg_offset, Zg_offset):
...
def outputPrcsPoints_l(lock,filePath,las_filename, x, y, z, gpstime, intens, retur, dist, heading, azi, totalAccel, ZangRate, deltaRoll, horVelo, upVelo):
...
def outputPrcsPoints_l_verbose(lock,filePath,las_filename, x, y, z, gpstime, intens, x0, y0, z0, R, P, H, Xg, Yg, Zg, Lat, Lon, Xg_offset, Yg_offset, Zg_offset):
    ...
    lasfile.gps_time = np.asarray(gpstime,dtype=np.float64)
    lasfile.intensity = intens
    xm = np.asarray(x0,dtype=np.float32)
...
def georefChunk_livox(statusD, statsD, start_time_ref, lock, chunkID, pointPackets, firmware, fileExtension, chunkFileName, params, navinterpSinLat, navinterpCosLat, navinterpSinLon, navinterpCosLon, navinterpEllH, navinterpGeoidSep, navinterpSinR, navinterpCosR, navinterpSinP, navinterpCosP, navinterpSinH, navinterpCosH, navinterpXg, navinterpYg, navinterpZg, navinterpTa, navinterpZar, navinterpDr, navinterpVh, navinterpVu, firstNavTime, lastNavTime, pcap_filename, quick, verbose, X_dict, Y_dict, time_dict, posPack_dict, badPPSmessages_dict, logMessages_dict, geo_epsg, proj_epsg, Xg_offset, Yg_offset, Zg_offset, filePath):
    ...
    lock.acquire()
    pbar = tqdm(total=len(pointPackets),unit=" packets",desc=descStr,position=chunkID,ncols=85)
    lock.release()
    time.sleep(0.05)
    
    start_day_ref = datetime(start_time_ref.year,start_time_ref.month,start_time_ref.day)
        ...
        if timestamp_type == 4:
            ...
            if datatype == 1:
                ...
                for j in range(0,100):
                    ...
                    if distance > 0.0:
                        if points_count % point_every == 0:
                            time_in_cur_day = (pt_time - start_day_ref).total_seconds()
                            points_timing.append(time_in_cur_day)
                            points_azi.append(azimuth)
                            points_vert.append(zenith)
                            points_dist.append(distance)
                            points_intens.append(reflectivity)
                            points_return.append(returnNum)
                    points_count += 1
    ...
    if chunkID == numProcesses-1:
        lock.acquire()
        print("\n            Georeferencing valid point observations, please wait ...\n")
        lock.release()
        time.sleep(0.1)
    ...
    if len(points_timing) > 0:
        ...
        if verboseOut == False:
            if fileExtension == ".LAS" or fileExtension == ".LAZ":
                timeMin, timeMax, logMessStr = outputPrcsPoints_l(lock,filePath,chunkFileName,X,Y,Z,points_timing,points_intens,points_return,points_dist,H,points_azi,totalAccel,ZangRate,deltaRoll,horVelo,upVelo)
            else:
                logMessStr = outputPrcsPoints_a(lock,filePath,chunkFileName,X,Y,Z,points_timing,points_intens,points_return,points_dist,H,points_vert,totalAccel,ZangRate,deltaRoll)
        else:
            if fileExtension == ".LAS" or fileExtension == ".LAZ":
                ...
                logMessStr = outputPrcsPoints_a_verbose(lock,filePath,chunkFileName,X,Y,Z,points_timing,points_intens,points_return,points_dist,points_vert,points_azi,x0,y0,z0,R,P,H,Xg,Yg,Zg)
                # logMessStr = outputPrcsPoints_a_verbose2(lock,filePath,chunkFileName,X,Y,Z,points_timing,points_return,x0,y0,z0,R,P,H,Xg,Yg,Zg,Lat,Lon,Xg_offset,Yg_offset,Zg_offset)
            
        timeInfo = str(timeMin) + "," + str(timeMax) + ",0"
        
        logMessStr = "                CHUNK " + chunkFileName + " is being PROCESSED ...\n" + logMessStr
    
                        
    else:
        lock.acquire()
        print("                CHUNK " + chunkFileName + " is OUTSIDE THE VALID NAV TIMES")
        lock.release()
        time.sleep(0.1)
        logMessStr = "                CHUNK " + chunkFileName + " is OUTSIDE THE VALID NAV TIMES\n" + logMessStr
        
    time_dict[chunkID] = timeInfo
    # X_dict[chunkID] = packet_timings
    # Y_dict[chunkID] = num_points_per_packet
    # posPack_dict[chunkID] = position_packets
    # badPPSmessages_dict[chunkID] = badPPSmessages
    logMessages_dict[chunkID] = logMessStr


def georef_project(params_filename, nav_filename, pcap_filename, livox_filename, las_filename, log_filename, quick, verbose, start_time, end_time, logFile, geo_epsg, proj_epsg, geocs_name, projcs_name, semi_major, semi_minor, filePath, pcapName, navName, livoxName, time_sync_corr):
    ...
    if fileExtension == ".LAS" or fileExtension == ".LAZ" or fileExtension == ".CSV":
        ...
        print("            Reading Lidar Data File: " + pcapName)
...
#        sys.stdout = logFile
        ...
        with open(livox_filename, 'r') as livox_file:
            ...
            time_ref = datetime(year=yyyy,month=mm,day=dd,hour=hh,minute=mins,second=secs_i,microsecond=us) + timedelta(seconds=time_sync_corr)
    
        epoch = time_ref    
        
        #time offset from when the next PPS pulse was detected to the start of data file opening
        # could/should be used to check that a confident syncronized start time has been determined
        ...
        if Success:
            ...
            print("            Reading binary packet data from PCAP file\n")
            pbar = tqdm(total=pcapSize,unit_scale=True,unit_divisor=0.001e6,unit='B',desc="                ",ncols=85)
        
            # read through data packets in PCAP file
            while True:
                
                # #debugging purposes
                # if bytesRead > 50000000:
                #     break
            ...
            if foundStuff:
                ...
                print("\n            Parsing observations from binary packet data using " + str(len(chunkIndex)-1) + " CPU core(s)\n")
                ...
                for i in range(0,len(chunkIndex)-1):
                    ...
                    proc1 = mp.Process(target=georefChunk_livox,args=(statusD, statsD, pointPackets[0][0], lock, i, pointPackets[chunkIndex[i]:chunkIndex[i+1]], firmware, fileExtension, chunkFileName, params, navinterpSinLat, navinterpCosLat, navinterpSinLon, navinterpCosLon, navinterpEllH, navinterpGeoidSep, navinterpSinR, navinterpCosR, navinterpSinP, navinterpCosP, navinterpSinH, navinterpCosH, navinterpXg, navinterpYg, navinterpZg, navinterpTa, navinterpZar, navinterpDr, navinterpVh, navinterpVu, firstNavTime, lastNavTime, pcap_filename, quick, verbose, X_dict, Y_dict, time_dict, posPack_dict, badPPSmessages_dict, logMessages_dict, geo_epsg, proj_epsg, Xg_offset, Yg_offset, Zg_offset, filePath))                    
                    procs.append(proc1)
...
if __name__ == "__main__":
    ...
    try:
        ...
        time_sync_corr = int(time_sync_corrStr)
    except:
        pass
        ...
        if proj_epsg != -1:
            ...
            if foundGeoCS:
                if foundProjCS:
                    ...
                    livoxPath = Path(livox_filename)
                    pcapName = pcapPath.name
                    navName = navPath.name
                    livoxName = livoxPath.name
                    filePath = pcapPath.parents[0]
                    ...
                    startPCP, endPCP, outfileName, fileExtension, minTimeProc, X_dict, Y_dict, posPack_dict, rotRate, numCores, Success = georef_project(params_filename, nav_filename, pcap_filename, livox_filename, las_filename, log_filename, quick, verbose, start_time, end_time, logFile, geo_epsg, proj_epsg, geocs_name, projcs_name, semi_major, semi_minor, filePath, pcapName, navName, livoxName, time_sync_corr)
